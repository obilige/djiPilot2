...
const onStart = async () => {
  ...
  switch (livetypeSelected.value) {
    case 1: {
      ...
      console.log("liveURL 111", liveURL);
      break
    }
    case 2: {
      // RTSP
  ...
  await startLivestream({
    ...
    .then(res => {
      ...
      if (livetypeSelected.value === 3) {
        ...
        const url = res.data.url
        // const url = 'webrtc://osiii.com/live/livestream';
        const videoElement = videowebrtc.value
        ...
        const player = OvenPlayer.create("video-webrtc", {
            "autoStart": true,
            "autoFallback": true,
            "mute": false,
            "sources": [{
                "type": "webrtc",
                "file": "ws://211.189.132.21:3333/app/stream"
            }]
        });
      }
